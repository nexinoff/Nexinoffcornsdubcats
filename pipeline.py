...
# Silero TTS — офлайн, бесплатно, без ключей
import torch
...
def synthesize_ru(text: str, out_wav: Path):
    if not text:
        # тишина, если распознать не получилось
        subprocess.run(
            ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo",
             "-t", "3", str(out_wav)], check=True,
        )
        return
    tts = _get_tts()
    audio = tts.apply_tts(text=text, speaker="baya", sample_rate=48000)
    import torchaudio
    torchaudio.save(str(out_wav), audio.unsqueeze(0), 48000)
...
def process_video(src_path: Path, out_path: Path, banner_path: Path | None = None,
    ...
    progress_cb("tts")
    ru_wav = work / "ru_voice.wav"
    synthesize_ru(ru_text, ru_wav)

    dubbed = work / "dubbed.mp4"
    progress_cb("mux")
    mux_new_audio(cropped, ru_wav, dubbed)
